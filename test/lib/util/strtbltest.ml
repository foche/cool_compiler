...
let%test "add" =
  ...
  Strtbl.find tbl hndl1 = "2" &&
  Strtbl.find tbl hndl2 = "ave123"
...
let%test "remove" =
  ...
  try Strtbl.find tbl hndl2 |> ignore; false with
  | Not_found -> Strtbl.find tbl hndl1 = "asdf"

let%test "negative create" =
  let tbl = Strtbl.create (-1) in
  let hndl1 = Strtbl.add tbl "" in
  let hndl2 = Strtbl.add tbl "ave123" in
  Strtbl.find tbl hndl1 = "" &&
  Strtbl.find tbl hndl2 = "ave123"
