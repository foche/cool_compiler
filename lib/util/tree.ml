...
let dfs ~out_edges ~visit_fun ~finish_fun ~init u =
  let rec aux parent state v =
    ...
    let state3 =
      Hashtbl.find_all out_edges v |> List.fold_left (aux v) state2
    in
...
let find_out_edges g = Hashtbl.find_all g.out_edges

let mem g u = u = g.root || Hashtbl.mem g.parents u
...
let find_parent_opt g = Hashtbl.find_opt g.parents
...
let all_lca g vlist = List.fold_left (lca g) (List.hd vlist) (List.tl vlist)
