...
let get_arith_f op x y =
  IArith (get_arith_op op, x, y)
...
let get_comp_f op x y =
  IComp (get_comp_op op, x, y)
...
let rec trans_exp args dest parent (exp, _ as exp_node) =
  let args', rhs =
    ...
    | Eq (e1, e2) -> trans_binop args exp_node e1 e2 (fun x y -> IComp (IEq, x, y))
...
and trans_var args id =
  args, IReg (make_var args id true)
...
and trans_dyn args parent dyn =
  ...
  (recv, temps) |> ignore; args'', zero_val
...
and trans_stat args parent stat =
  ...
  let args''' = {
      args'' with
      block = Call (get_opt stat.stat_label, recv :: Array.to_list temps) :: args''.block
    } in
  args''', IReg RetReg

(* TODO *)
and trans_cond args parent if_node then_node else_node =
  if_node |> ignore; then_node |> ignore; else_node |> ignore; parent |> ignore; args, zero_val

(* TODO *)
and trans_loop args parent pred_node body_node =
  pred_node |> ignore; body_node |> ignore; parent |> ignore; args, zero_val
