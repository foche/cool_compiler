...
let run_on_file f acc filename =
  ...
  let acc' =
    try f acc filename file with
      | ex -> close_in file; raise ex in
  close_in file;
  acc'
...
let parse_single (acc, empty_count) filename file =
  ...
  buf.lex_curr_p <- {
    buf.lex_curr_p with pos_fname = filename
  };
  buf.lex_start_p <- {
    buf.lex_start_p with pos_fname = filename
  };

  Coollexer.print_filename filename;
  let cls_opt, is_empty =
    try Coolparser.parse Coollexer.get_token buf with
    | Parsing.Parse_error -> None, false in

  let new_count = empty_count + (if is_empty then 1 else 0) in
  cls_opt :: acc, new_count

let internal_parse filenames =
  let cls_opts, empty_count = List.fold_left (run_on_file parse_single) ([], 0) filenames in
  let cls = List.rev_map (get_opt ~default:(Some [])) cls_opts |> List.concat in
  ...
  | true, _ -> List.hd filenames |> Astprint.print_eof_error; None
  | false, true -> None
  | false, false ->
