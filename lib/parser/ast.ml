...
and typed_expr = {typ_expr: expr; typ_type: type_sym option}

and expr_node = typed_expr with_loc

and dynamic_dispatch =
  {dyn_recv: expr_node; dyn_method: id_sym; dyn_args: expr_node list}
...
type feature = Method of mthd | Field of id_sym * type_sym * expr_node
...
let make_expr ~exp ~typ = {typ_expr= exp; typ_type= typ}

let make_untyped_expr ~exp n = (make_expr ~exp ~typ:None, n)
...
let make_let bindings body =
  ...
  let init =
    make_untyped_expr
      ~exp:
        (Let {let_var= x; let_var_type= typ; let_init= init; let_body= body})
      n
  in
...
let translate_type cl typ =
  match typ = Tables.self_type with true -> cl.class_type | false -> typ
