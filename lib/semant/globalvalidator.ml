...
let validate_method_types graph (cl, _) =
  let is_valid_type typ =
    typ = object_type || Hashtbl.mem graph typ in
  ...
  let check_field_type line_number id typ =
    ...
    | false ->
      ...
      Printf.eprintf
        "Class %a of attribute %a is undefined.\n"
        print_type typ
        print_id id;
      false in
  ...
  let check_formal ((id, typ), line_number) =
    ...
    | false ->
      Semantprint.print_location cl.class_filename line_number;
      Printf.eprintf
        "Class %a of formal parameter %a is undefined.\n"
        print_type typ
        print_id id;
      false in
  ...
  let check_ret_type typ line_number method_id =
    ...
    | false ->
      Semantprint.print_location cl.class_filename line_number;
      Printf.eprintf
        "Undefined return type %a in method %a.\n"
        print_type typ
        print_id method_id;
      false in

  let check_method mthd line_number =
    let valid_ret_type = check_ret_type mthd.method_ret_type line_number mthd.method_id in
    let valid_formals = List.for_all check_formal mthd.method_args in
    valid_ret_type && valid_formals in

  let check_feature (feat, line_number) =
    match feat with
    | Method mthd -> check_method mthd line_number
    | Field (id, typ, _) -> check_field_type line_number id typ in
...
let validate_formal_not_self filename ((id, typ), line_number) =
  ...
  let is_valid_type =
    ...
    | false ->
      Semantprint.print_location filename line_number;
      Printf.eprintf
        "Formal parameter %a cannot have type SELF_TYPE.\n"
        print_id id;
      false in
...
let add_to_sigs sigs cl mthd line_number =
  ...
  Methodtbl.add
    ~tbl:sigs
    ~clazz:cl.class_type
    ~method_id:mthd.method_id
    ~return_type:mthd.method_ret_type
    ~formals in
...
let validate_feature args cl (feat, line_number) =
  ...
  | Method mthd ->
    ...
    let no_self_args =
      List.for_all (validate_formal_not_self cl.class_filename) mthd.method_args in
    is_unique && is_valid_main && no_self_args

let add_to_graph graph cl line_number =
  match Hashtbl.mem graph cl.class_type with
  | false -> Hashtbl.add graph cl.class_type cl.class_parent; true
  | true ->
    Semantprint.print_location cl.class_filename line_number;
    Printf.eprintf
...
let is_valid_inheritance inheritance_blocklist cl line_number =
  ...
  | true ->
    Semantprint.print_location cl.class_filename line_number;
    Printf.eprintf
      "Class %a cannot inherit class %a.\n"
      print_type cl.class_type
      print_type cl.class_parent;
    false
...
let validate_class args (cl, line_number as cl_node) =
  ...
  let is_valid_parent = is_valid_inheritance args.inheritance_blocklist cl line_number in
  let is_unique = is_valid_type && add_to_graph args.graph cl line_number in
  let valid_features = List.for_all (validate_feature args cl) cl.class_features in
...
let create_tree graph handle_to_class =
  ...
  | Tree.Disconnected (typ, parent) ->
    ...
    Semantprint.print_location cl.class_filename line_number;
    Printf.eprintf
      "Class %a inherits from an undefined class %a.\n"
      print_type typ
      print_type parent;
    false, None
...
let validate args =
  ...
  | false -> false, None
  | true -> create_tree args.graph args.handle_to_class in
  let valid_method_types = List.for_all (validate_method_types args.graph) classes in
