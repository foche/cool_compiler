      ...
      | Some init_typ -> (
        ...
        | false ->
            ...
            Printf.eprintf
              ...
              print_type init_typ print_id id print_type typ ;
            None ) )

let typecheck_method args cl mthd line_number =
  let add_formal ((id, typ), line_number') =
    match Symtbl.add args.id_env ~k:id ~v:typ |> fst with
    | false -> true
    | true ->
        Semantprint.print_location cl.class_filename line_number' ;
    ...
    | false ->
        ...
        Printf.eprintf
          "Inferred return type %a of method %a does not conform to declared \
           return type %a.\n"
          print_type typ print_id mthd.method_id print_type
          mthd.method_ret_type ;
        None )
...
let validate_formal_types filename method_id ((_, typ), line_number)
    ((_, parent_typ), _) =
  ...
  | false ->
      ...
      Printf.eprintf
        "In redefined method %a, parameter type %a is different from original \
         type %a.\n"
        print_id method_id print_type typ print_type parent_typ ;
      false

let validate_method_sig filename line_number mthd parent_method =
  let param_counts_match =
    List.compare_lengths mthd.method_args parent_method.method_args = 0
  in
  let return_types_match =
    mthd.method_ret_type = parent_method.method_ret_type
  in
  let formal_types_match =
    param_counts_match
    && List.for_all2
         (validate_formal_types filename mthd.method_id)
         mthd.method_args parent_method.method_args
  in
  ...
  | false ->
      ...
      Printf.eprintf
        "In redefined method %a, return type %a is different from original \
         return type %a.\n"
        print_id mthd.method_id print_type mthd.method_ret_type print_type
        parent_method.method_ret_type ) ;
  param_counts_match && return_types_match && formal_types_match

let extract_method func_env filename line_number mthd =
  let parent_method_opt = Symtbl.find_opt func_env ~k:mthd.method_id in
  let _, is_overridden = Symtbl.add func_env ~k:mthd.method_id ~v:mthd in
...
let extract_field id_env cl line_number id typ =
  let is_duplicate, is_shadowed = Symtbl.add id_env ~k:id ~v:typ in
  ( match is_duplicate with
  | false -> ()
  | true ->
      Semantprint.print_location cl.class_filename line_number ;
      Printf.eprintf "Attribute %a is multiply defined in class %a.\n" print_id
        id print_type cl.class_type ) ;
...
let typecheck_class args typ (cl, line_number) =
  ...
  | true -> (
      ...
      | Some typed_features ->
          let typed_class =
            ({cl with class_features= typed_features}, line_number)
          in
         ...
         ~cont:( lazy
            ( ( Hashtbl.mem args.ignored_classes root
              || Hashtbl.find args.untyped_classes root
                |> typecheck_class args root )
            && Tree.find_out_edges args.graph root
              |> List.for_all (traverse_class_tree args) ) )) )

let validate args =
  Symtbl.add args.id_env ~k:self_var ~v:self_type |> ignore ;
